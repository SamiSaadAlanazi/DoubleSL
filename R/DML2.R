...
#' The Double Machine-Learning Method (DML): Binary Case Treatment
#' @description  Estimates the treatment effect using the DML method when the treatment variable is considered to be binary.
#' @export
#' @param data The data set. Treatment variable D must be binary, and the number of variables must be greater than 3.
...
DML2 <- function(data, y.col.n, d.col.n) {
  R=y.col.n
  Tr=d.col.n
  N=nrow(data)
  DAT=data
  ...
  modeld1=randomForest(DAT.IC[,-c(R, Tr)],as.factor(DAT.IC[,Tr]))
  modeld2=randomForest(DAT.I[,-c(R, Tr)],as.factor(DAT.I[,Tr]))
  M1=predict(modeld1,DAT.I[,-c(R, Tr)], type='prob')
  M2=predict(modeld2,DAT.IC[,-c(R, Tr)], type='prob')
  V1=DAT.I[,Tr]-M1[,2]
  V2=DAT.IC[,Tr]-M2[,2]
  # Compute Cross-Fitting DML estimator of theta #
  theta1=mean(V1*U1)/mean(V1*DAT.I[,Tr])
  theta2=mean(V2*U2)/mean(V2*DAT.IC[,Tr])
